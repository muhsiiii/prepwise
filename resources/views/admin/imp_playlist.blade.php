    ...
    <div class="col-12">
        <div class="card card-primary">
            <div class="card-header">
                <h3 class="card-title">Playlist Title List ({{ $impplaylists->total() }})</h3>
            </div>
            <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap table-bordered table-extra" id="rtable"
                    ...
                    <tbody>
                        @foreach ($impplaylists as $impplaylist)
                        @php
                        $iteration = ($impplaylists->currentPage() - 1) * $impplaylists->perPage() + $loop->index + 1;
                    @endphp
                            <tr>

                                <td align="center">{{ $iteration }}</td>
                                <td align="center">{{ $impplaylist->playlist }}</td>
                                <td align="center">
                                    <a class="btn btn-info btn-sm text-light" onclick="PlaylistEdit('{{ $impplaylist->id }}','{{ $impplaylist->playlist }}')">Edit</a>
                                    <a class="btn btn-danger btn-sm text-light" onclick="Playlistdelete('{{ $impplaylist->id }}')">Delete</a>
                                </td>
                            </tr>
                        @endforeach
